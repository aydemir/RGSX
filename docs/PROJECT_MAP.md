...
| Rust gap'leri (Faz 13) | `tasks/gap/TASK-002-gap-*.md`, tamamlanan `tasks/in-progress/` | Python iş akışında (PYTHON_WORKFLOW.md) Rust karşılığı OLMAYAN düğümler: retry engine, cancel+temizlik, HTTP-direct, disk alanı, extract, seed lifecycle, stray-temp, restart-resume, history/SSE, 1fichier provider zinciri. ✅ **Gap 2 (pause/resume) tamamlandı** (3ba8a8e, `tasks/in-progress/`). P0→P2 sıralı (Faz 13 roadmap) |
